...
class UserService implements UserServiceImpl
{
    ...
    {
        $response = $this->model->updateUserAccount($payload);
        if ($response) {
            return ResponseDto::json("User account information was updated successfully", 200);
        }
        return ResponseDto::json("We are unable to update your information. Please try again!");
    }

    function filterValidMembers(array $usernames)
    {
        $valid_usernames = [];
        foreach ($usernames as $key => $username) {
            $res = $this->model->isUsername($username);
            if ($res) array_push($valid_usernames, $res["user_id"]);
        }
